...
async def chat_gpt(client, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            response = requests.get(f'https://api.botcahx.eu.org/api/linkshort/tinyurl?link={a}&apikey=ZYuQf2uF')
...
async def chat_gpt(client, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            prs = await message.reply_text(f"<emoji id=5071138963800982678>😎</emoji> Proses....")
            a = message.text.split(' ', 1)[1]
            response = requests.get(f'https://api.botcahx.eu.org/api/linkshort/bitly?link={a}&apikey=ZYuQf2uF')
