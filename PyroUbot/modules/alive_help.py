...
<blockquote>{bot.me.mention}
    status : {status} 
       {psr} expired_on : {exp} 
        ...
        <blockquote><b>USERBOT PREM NEO FETCH</b></blockquote>
"""
...
async def _(client, callback_query):
    ...
    os.execl(sys.executable, sys.executable, "-m", "ᴜʙᴏᴛᴘᴜᴛʀᴀɢᴀɴᴀ-ᴘʀᴇᴍ")
...
async def user_help_inline(client, inline_query):
    ...
    caption = (
        ...
        f"ᴘʀᴇꜰɪx: {' '.join(SH)}\n"
        f"ᴍʏ ᴜʙᴏᴛ: <a href=t.me/botcreateubot_bot>botcreateubot_bot</a>"
        f"</b></blockquote>"
    )

    results = [
        InlineQueryResultPhoto(
            photo_url="https://files.catbox.moe/36qaap.jpg",
            thumb_url="https://files.catbox.moe/36qaap.jpg",
            caption=caption,
